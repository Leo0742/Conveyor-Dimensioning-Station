...
Все опубликованные метрики получены на синтетических данных. Я не испытывал
физический Gocator и конвейер, поэтому не делаю выводов о точности реальной станции.

Выбранная архитектура — один двухкамерный профилометр **LMI Gocator 2880**.
Итоговый результат на синтетике: **93,6%** результатов со статусом `OK`
уложились в допуск. Проверено 500 случайных сценариев методом Monte Carlo;
начальное значение генератора — `20261018`.
...
минимального объёма. В программном прототипе используется приближённый OBB; перед
физической приёмкой его ошибку нужно сравнить с точной или эталонной реализацией
параллелепипедом минимального объёма.
...
Программный прототип в этом репозитории не подключён к реальным WLF4FI, DFS60 и Gocator.
...
→ оценка контакта с лентой → приближённый OBB → проверка качества
→ JSON для WMS и локальная JSONL-очередь
```

В этом репозитории я проверяю геометрию и программный путь обработки. Аппаратный
адаптер Gocator/PLC и передачу данных в реальную WMS нужно будет реализовать на
следующем этапе.
...
- структура для передачи точек, положения энкодера и признаков качества без
  имитации реального драйвера;
- регрессионные тесты, тест упрощённой модели сенсора, 500 случайных сценариев
  и проверка объединения нескольких окон.

Прототип передаёт в WMS отсортированные рёбра OBB: `length_mm ≥ width_mm ≥ height_mm`.
Это не привязка к X/Y/Z. Вертикальный габарит станции отдельно ограничен 300 мм;
если реальная WMS понимает `height_mm` именно как ось Z, понадобится другое явное
сопоставление полей.

Итог по нескольким окнам использует только окна `status=ok`: требуется минимум 3 окна,
...
На зафиксированном синтетическом наборе (начальное значение генератора `20261018`):
...
Предыдущее начальное значение `20261017` больше не используется, потому что после
исправления размещения товара на ленте изменился генератор сценариев. До запуска
с новым значением `20261018` я зафиксировал параметры и после получения результата
не менял пороги.

Хуже всего алгоритм работает с наклонёнными и нерегулярными объектами: точность
среди принятых результатов составляет 89,4%, а доля корректных измерений — 74,4%.
В сценариях с шумом и неполными данными точность среди принятых равна 84,0%.
...
В базовой установке Open3D не ставится специально, поэтому один дополнительный
тест будет пропущен. Это нормально: полная проверка с Open3D описана ниже.
...
изменяют сохранённые файлы демонстрации.
...
параллелепипеда минимального объёма.
...
assets/demo/                зафиксированные файлы демонстрации из отчёта
...
с опорной плоскостью остаётся эвристикой. OBB в прототипе — приближённый, а `confidence` —
некалиброванный показатель качества, не вероятность. Реальная отправка по HTTPS/mTLS
не реализована: прототип формирует WMS-сообщение и локальную очередь. Абсолютную
...
варианты замены — другая геометрия установки, меньшее световое пятно, световая
завеса или определение присутствия товара по PLC либо потоку профилей.

Подробное [сравнение оборудования](docs/hardware_trade_study.md), схема станции,
ограничения по точности, возможное применение нейросети и план физической проверки собраны
